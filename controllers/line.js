...
const getLineData = async (req, res) => {
  ...
  try{
    ...
    rows.forEach(row => {
      // Create a date string for the year
      const dateString = `${row.year}-01-01`;  // Format: YYYY-MM-DD

      positiveData.push({ x: dateString, y: row.avg_positive_score });
      negativeData.push({ x: dateString, y: row.avg_negative_score });
      neutralData.push({ x: dateString, y: row.avg_neutral_score });
  });
